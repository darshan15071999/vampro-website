      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ...
            className={`fixed bottom-6 right-6 z-[60] h-14 w-14 rounded-2xl shadow-xl flex items-center justify-center hover:scale-105 transition-transform overflow-hidden p-1 backdrop-blur-md ${
              isLight ? 'bg-white/90 border border-slate-200 shadow-indigo-500/10' : 'bg-[#07060F]/80 border border-white/10 shadow-indigo-500/30'
            }`}
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed bottom-6 right-6 z-[60] w-full max-w-[90vw] sm:max-w-sm sm:w-96 border rounded-2xl shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl ${
              isLight ? 'bg-white/85 border-slate-200' : 'bg-[#07060F]/85 border-white/10'
            }`}
            style={{ maxHeight: '80vh', height: '600px' }}
          >
            {/* Header */}
            <div className={`flex items-center justify-between px-4 py-3 shrink-0 ${isLight ? 'bg-slate-50/50 border-b border-slate-200' : 'bg-white/5 border-b border-white/10'}`}>
            ...
            <div className={`flex-1 overflow-y-auto p-4 space-y-4`}>
              ...
              {/* Stacked Predefined Questions in Default View */}
            ...
            {/* Input Area */}
            <div className={`p-3 shrink-0 ${isLight ? 'bg-slate-50/50 border-t border-slate-200' : 'bg-white/5 border-t border-white/10'}`}>
              <form 
              ...
              >
                <input
                  ...
                  className={`flex-1 border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors backdrop-blur-sm ${
                    isLight 
                      ? 'bg-white/70 border-slate-300 text-slate-900 placeholder-slate-400'
                      : 'bg-black/20 border-white/20 text-white placeholder-slate-500'
                  }`}
                />
                <button
                  type="submit"
