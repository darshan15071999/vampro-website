...
const API_URL = "https://corsproxy.io/?https://integrate.api.nvidia.com/v1/chat/completions";
        ...
        {isOpen && (
          <motion.div
            ...
            className={`fixed bottom-6 right-6 z-[60] w-full max-w-[90vw] sm:max-w-sm sm:w-96 border rounded-2xl shadow-2xl flex flex-col overflow-hidden ${
              isLight ? 'bg-white border-slate-200' : 'bg-[#07060F] border-white/10'
            }`}
            style={{ maxHeight: '80vh', height: '600px' }}
          >
            {/* Header */}
            <div className={`flex items-center justify-between px-4 py-3 shrink-0 ${isLight ? 'bg-slate-50 border-b border-slate-200' : 'bg-[#07060F] border-b border-white/10'}`}>
            ...
            {/* Messages Area */}
            <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${isLight ? 'bg-white' : 'bg-[#07060F]'}`}>
            ...
            {/* Input Area */}
            <div className={`p-3 shrink-0 ${isLight ? 'bg-slate-50 border-t border-slate-200' : 'bg-[#07060F] border-t border-white/10'}`}>
              <form 
                ...
                <input
                  ...
                  className={`flex-1 border rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500 transition-colors ${
                    isLight 
                      ...
                      : 'bg-transparent border-white/20 text-white placeholder-slate-500'
                  }`}
                />
                <button
                  type="submit"
