...
const GhostCursor = ({
  ...
  function calculateScale(el: HTMLDivElement) {
  ...
  const mergedStyle = useMemo(() => ({ zIndex, position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', ...style }), [zIndex, style]);

  return <div ref={containerRef} className={`ghost-cursor ${className ?? ''}`} style={mergedStyle as any} />;
};
